...
public class controladorEditorAtividades {
	...
	public void apertarOK() {
		...
		try {
			...
			int tempoExecucao = hmsParaSec(Integer.parseInt(duracaoHora), Integer.parseInt(duracaoMin), Integer.parseInt(duracaoSec));
			int tempoPausa = hmsParaSec(Integer.parseInt(pausaMin), Integer.parseInt(pausaSec));
	...
	public void carregaAtividade(Perfil perfil, String atividade) throws ClassNotFoundException, NullPointerException, IOException {
		...
		Integer[] dHMS = secParaHMS(ativ.getDuracao());
		Integer[] pHMS = secParaHMS(ativ.getPausa());
	...
	public int hmsParaSec(int hr, int min, int sec) {
		return hr*3600 + min*60 + sec;
	}
	
	public int hmsParaSec(int min, int sec) {
		return min*60 + sec;
	}
